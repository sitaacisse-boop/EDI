...
\usepackage{xcolor} %highlight with color
...
% Short title
\shorttitle{Aggregating Preferences with EDI via Graph Summarization}

% Short author
\shortauthors{X}

% Main title of the paper
\title [mode = title]{Aggregating User Preferences while Ensuring Equity, Diversity, and Inclusion using Graph Summarization}
...
%\tnotetext[1]{An interactive dashboard with the full experimental results is available at \texttt{https://sitaacisse-boop.github.io/EDI/}}
...
\author[1]{}%[<options>]
%\author[inst1]{Adji Marieme Sita Cissé}
%\author[inst1]{Malek Mouhoub\corref{cor}}[orcid=0000-0001-7381-1064]
%\author[inst1]{X}
%\author[inst1]{Y\corref{cor}}
...
% Address/affiliation
\affiliation[inst1]{organization={Department of Computer Science, University of Regina},
            addressline={Wascana Parkway},
            city={Regina},
            postcode={S4S 0A2},
            state={Saskatchewan},
            country={Canada}}
%\affiliation[inst2]{organization={Department of Computer Science, University of Regina},
 %           addressline={Wascana Parkway},
  %          city={Regina},
   %         postcode={S4S 0A2},
    %        state={Saskatchewan},
...
meaningfully reduce the equity gap, a limitation we
...
the aggregation structure yields more robust fairness–diversity trade-offs
than post-hoc approaches, while also revealing that no single
compression budget or structural mechanism resolves the equity
–diversity tension uniformly across domains.
\end{abstract}
...
\begin{keywords}
graph coarsening \sep recommender systems \sep fairness \sep EDI metrics \sep
preference aggregation \sep bipartite graph \sep social choice
\end{keywords}
...
Addressing EDI in recommendations requires aggregating the preferences of many
...
al.~\citeyearpar{roy_fairness_2024} structure the problem along three dimensions,
...
Notably, Roy et al.~\citeyearpar{roy_fairness_2024} distinguish between making an
...
Building on this tradition, Kellerhals and Peters~\citeyearpar{kellerhals_proportional_2024}
...
Liu et al.~\citeyearpar{liu_graph_2018} provide a reference survey of the field and
...
al.~\citeyearpar{shabani_comprehensive_2024}, applies graph neural networks to
...
Huang~\citeyearpar{yao_new_2017} study fairness in collaborative filtering and show
...
al.~\citeyearpar{leonhardt_user_2018} focus on user-side fairness and provide
...
al.~\citeyearpar{zhao_fairness_2024} survey fairness and diversity in recommender
...
al.~\citeyearpar{dong_fairness_2023} survey fairness specifically in graph mining,
...
directly within recommendation. Aird et al.~\citeyearpar{aird_exploring_2023}
...
Huang~\citeyearpar{yao_new_2017}. For each group $g \in \{F, M\}$, the group utility
...
core concept of Kellerhals and Peters~\citeyearpar{kellerhals_proportional_2024} and
operationalized as the group proportional fairness metric of Aird et
al.~\citeyearpar{aird_social_2024}. Let $\theta$ be a relevance threshold and
...
threshold used by Aird et al.~\citeyearpar{aird_social_2024} to define a ``liked''
...
entirely excluded from the relevant items on the top-2 list.
...
For the main EDI-constrained coarsening experiments we use a random
subsample of 1,000 raters (97,707 ratings, Table~\ref{tab:datasets-overview});
Section~\ref{sec:libimseti-limitation} additionally analyzes the full
17.3M-edge graph to characterize rating behavior by gender pair. We set
$k \in \{5, 10, 20\}$.
...
Rate My Prof.     & Education    & 1,981 & 59,066 & 150,800    & 1--5  & item-side & 51\%* \\
OpenAlex          & Academic     & 99    & 904    & 2,124      & 1--5  & item-side & 19\% \\
\hline
\end{tabular}
}
\vspace{2pt}
...
a relevance threshold of $\theta \in \{3.5, 4.0\}$ for MovieLens and
...
MovieLens 100k). This is roughly $4.0$--$4.4\times$ slower than Fair Re-rank
...
which range from 0.716 to 0.746), but does not meaningfully reduce $\Delta E$
relative to Borda (1.232 vs.\ 1.301, both well above 1.2).
...
Huang~\citeyearpar{yao_new_2017}: demographic parity ($\Delta E \approx 0$) is not
...
narrow, cherry-picked setting, we re-ran AURORA at four budgets — 30\%,
...
\begin{tabular}{lcccc}
\hline
Budget   & \multicolumn{2}{c}{MovieLens 1M ($k=20$)} &
\multicolumn{2}{c}{Rate My Professors} \\
($\%$ of $|U|$) & $\Delta E \downarrow$ & ILD $\uparrow$ & $\Delta E \downarrow$ &
ILD $\uparrow$ \\
\hline
...
as the budget grows — substantially for $\Delta E$ (0.332 at 30\% to 0.185
at 60\%, a 44\% reduction) and marginally for ILD (0.456 to 0.470) — the
method is robust to the exact choice of budget, and 50\% is a reasonable,
round value within a range that performs consistently well. On Rate My Professors, ILD is stable and
...
advantage over the classical voting rules is not an artifact of the
...
This paper addresses the problem of aggregating user preferences in a way that
explicitly preserves equity, diversity, and inclusion. We represent preferences
as a weighted attributed bipartite graph and propose AURORA, an EDI-constrained
...
graph, AURORA—like the classical voting rules—does not meaningfully reduce
the equity gap, a limitation traced to a genuine difference in rating behavior
between groups rather than an algorithmic shortcoming. On the item-side datasets, AURORA shows
