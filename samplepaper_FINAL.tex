...
\begin{document}
%
\title{Aggregating User Preferences while Ensuring Equity, Diversity, and Inclusion using Graph Summarization}
%
...
Supervised by Prof.\ Malek Mouhoub}
...
to prevent results from systematically favouring majority groups, collapsing
onto homogeneous items, or under-representing minorities. These tensions
surface concretely in group recommendation, where optimising for utility
alone entrenches majority bias and filter bubbles. We address this problem
through EDI-constrained graph summarization. User preferences are modelled
...
achieves the lowest equity gap ($\Delta E = 0.051$) on OpenAlex, where
...
bipartite graph, no method reduces the equity gap, a limitation we
analyse and connect to prior findings that demographic parity is not
...
to minimise structural error, we constrain the summarisation so as to bound
...
and sums them; \emph{Weighted Borda} normalises Borda scores by group size
...
so that no group is systematically favoured or disadvantaged by the collective
...
summarising the preference graph, and the precise metrics used to quantify
...
as our default and compare with $\theta = 3.5$ experimentally.
...
attribute is the gender of the professor (item-side equity). We use $k = 10$.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_data_overview}
\caption{MovieLens 100k overview: rating distribution, gender split
(273 F / 670 M), and number of ratings per user.}
\label{fig:data}
\end{figure}
...
and sums them; (3)~\textbf{Weighted Borda}, which normalises Borda scores
...
to maximise the sum of group inclusion scores while minimising the equity gap.
...
relevance threshold $\theta \in \{3.5, 4.0\}$ for MovieLens. For item-side
equity datasets (Rate My Professors, OpenAlex), we report $\Delta E$, ILD,
and $\mathrm{frac}_F$ — the fraction of female items in the top-$k$ list.
...
graph. We set $k \in \{5, 10, 20\}$ for MovieLens and $k = 10$ for all other
datasets, and report the compression ratio $|U'|/|U|$ alongside all EDI metrics.
...
Borda         & 0.423 & 0.376 & 0.247 & 0.339 & -- \\
Weighted Borda & 0.393 & 0.390 & 0.255 & \textbf{0.344} & -- \\
Condorcet     & 0.441 & 0.380 & 0.244 & 0.340 & -- \\
Fair Re-rank  & 0.240 & 0.427 & 0.258 & 0.310 & -- \\
AURORA          & 0.285 & 0.462 & \textbf{0.277} & 0.344
              & \textbf{0.499} \\
\hline
$\alpha_g$ target & -- & -- & 0.29 & 0.71 & -- \\
\hline
...
$k$. At $k=10$, AURORA is the only method to improve all four metrics
over Borda simultaneously: Weighted Borda ties Borda exactly at this $k$
(identical top-$k$ item set), while Condorcet and Fair Re-rank each fail
on one criterion ($\Delta E$ and $\mathrm{inc}_F$ respectively). At
$k=20$ it improves $\Delta E$, ILD, and $\mathrm{inc}_F$ over
Borda and Condorcet simultaneously, and over Weighted Borda on all but
$\mathrm{inc}_M$, where the two methods are within 0.001 of each other ---
a property it shares only with Weighted Borda among the classical rules,
though Weighted Borda achieves it by directly reweighting scores by group
size, whereas AURORA requires no such explicit demographic reweighting of
the aggregation rule itself. On ILD specifically, AURORA
...
Fair Re-rank, AURORA's advantage only materialises at $k=20$ (0.462 vs.\
...
and $30$--$60\times$ slower than Borda, but remains on the order of a
...
whereas Fair Re-rank (0.144) is a clear outlier among the non-degenerate
...
all around 0.744--0.746), but does not reduce $\Delta E$ relative to Borda.
We analyse this limitation in detail in
...
behaviour between groups rather than an algorithmic shortcoming that
...
approaching the reduction seen on the other three datasets.
...
on equity: AURORA's $\Delta E$ (0.112) is higher than Borda's (0.008) and
further from the near-zero gap achieved by Average Score and Fair Re-rank
(both 0.000), so the method should be read as trading a real amount of
equity for a large gain in diversity and representation on this dataset.
Fair Re-rank raises $\mathrm{frac}_F$ to 0.50 but at the
cost of dramatically lower diversity (ILD\,$=$\,0.196), confirming that
post-hoc re-ranking cannot jointly optimise diversity and item-side
inclusion the way structural coarsening does.
...
while our method achieves the lowest equity gap ($\Delta E = 0.051$, a 28\%
improvement over Average Score's 0.071) while maintaining comparable female
representation ($\mathrm{frac}_F = 0.197$). Here, unlike on RMP, AURORA
improves equity \emph{and} maintains diversity simultaneously, confirming
that EDI-constrained graph coarsening generalises meaningfully to item-side
...
why no method --- including AURORA --- meaningfully reduces $\Delta E$ on this
...
remains close to $\Delta E \approx 0.008$ throughout this range on Rate My
...
Table~\ref{tab:synthesis} summarises the best-performing method per
...
RMP      & Avg Score / F.R. & Avg Score & \textbf{AURORA} & Trade-off: AURORA (div.) / F.R. (eq.) \\
...
with MovieLens 100k: Weighted Borda improves over Borda on $\Delta E$ (0.538
$\to$ 0.371 at $k=10$), and AURORA beats all three voting rules (Borda, Weighted
...
$k=20$, where it achieves the highest $\mathrm{inc}_F$ of any method (0.294,
versus 0.290 for Fair Re-rank and 0.284--0.287 for the voting rules),
suggesting that its advantage is concentrated in balancing inclusion across
groups rather than in minimising $\Delta E$ or maximising ILD outright at
...
Evaluated across five datasets spanning four domains, our experiments reveal
consistent patterns alongside domain-specific nuances. On MovieLens 100k with
$k \in \{5, 10, 20\}$, no single baseline simultaneously satisfies all three EDI
criteria. Average Score achieves the lowest equity gap but produces near-zero
inclusion for both groups, rendering it degenerate as a fairness-aware method.
Fair Re-rank reduces $\Delta E$ most effectively among non-degenerate methods,
yet at the cost of lower diversity at $k=5$ and $k=10$. AURORA beats Borda and
Condorcet on $\Delta E$ at every $k$, and on ILD at every $k$ except $k=5$
against Condorcet; against Weighted Borda, it improves $\Delta E$ and ILD at
every $k$ but trails marginally on $\mathrm{inc}_M$ at $k=20$ (0.344 vs.\ 0.344,
a 0.0007 gap). At $k=10$, AURORA is the only method to improve all four metrics
over Borda simultaneously (Weighted Borda ties Borda exactly at this $k$;
Condorcet and Fair Re-rank each fail on one criterion). At $k=20$, AURORA
simultaneously improves $\Delta E$, ILD, and $\mathrm{inc}_F$ over both Borda
and Condorcet --- a property shared only with Weighted Borda among the classical
rules, though Weighted Borda achieves it by directly reweighting scores by
group size, whereas AURORA requires no such explicit demographic reweighting
of the aggregation rule itself --- alongside a merge budget fixed at 50\% of
$|U|$ that we validated for robustness across a 30--60\% range. On MovieLens 1M,
this picture shifts: Fair Re-rank dominates AURORA on both $\Delta E$ and ILD
at every $k$, though AURORA still beats every classical voting rule on both
axes and achieves the best group inclusion of any method at $k=20$. This is a
genuine, dataset-dependent result rather than a uniform advantage, and we
report it as such.

On libimseti, the dense, reciprocally-structured social graph limits what
compression can achieve: AURORA improves diversity over the voting rules but,
like them, does not reduce $\Delta E$ below Average Score's degenerate
baseline. We traced this to a structural property of the domain --- rating
patterns differ substantially by rater--profile gender pair, so a single
collective ranking cannot equally satisfy every group --- rather than to a
shortcoming of the coarsening mechanism itself, echoing Yao and Huang's
argument that demographic parity is not always an appropriate target when
group preferences genuinely diverge. On Rate My Professors, AURORA uniquely
combines high diversity (ILD $=$ 0.808) with the highest female
item representation ($\mathrm{frac}_F = 0.60$), at the cost of a higher
$\Delta E$ than Borda (0.112 vs.\ 0.008) --- a trade-off, not a strict
dominance. On OpenAlex, where Borda-based methods fail entirely by
recommending zero female authors, AURORA achieves the lowest equity gap
($\Delta E = 0.051$, down 28\% from Average Score) while maintaining
comparable female representation, without the equity--diversity trade-off
observed on RMP.

Taken together, these results support a more precise claim than "AURORA
is uniformly better": embedding EDI constraints into the coarsening
structure consistently outperforms unconstrained voting rules, and matches
or improves on post-hoc re-ranking on smaller or item-side datasets, but a
sufficiently large user base (MovieLens 1M) or a domain with reciprocal,
attribute-dependent preferences (libimseti) can favour a post-hoc approach
or expose a genuine limit of any single-ranking aggregation framework,
respectively. This nuance is itself a contribution: it clarifies the
...
Several directions remain open for future work. First, extending the approach
to multiple simultaneous sensitive attributes --- such as gender combined with
age or occupation --- would increase the practical
applicability of the framework. Second, the current algorithm operates on a
static snapshot of the preference graph; adapting it to dynamic or streaming
settings, where user preferences evolve over time, is an important challenge
for real-world deployment. Third, the tolerances
$\varepsilon_E$, $\varepsilon_D$, and $\varepsilon_I$, and the merge budget
itself, are currently set empirically and validated by a robustness sweep
rather than derived from a principled criterion; a Pareto-frontier analysis
across the three EDI objectives, or a validation
split distinct from the reported test data, would make the calibration more
rigorous. Fourth, understanding why Fair Re-rank overtakes AURORA on
MovieLens 1M but not on the smaller datasets --- whether this is driven by
user-base size, graph density, or the candidate pool available to the
re-ranking step --- is an open question that would clarify the boundary
conditions under which structural coarsening is the preferable mechanism.
Fifth, on Rate My Professors specifically, merging course-side (user-side)
nodes does not directly control the gender composition of the recommended
professors (item-side); a merge criterion that accounts for the downstream
item distribution, rather than rating-profile similarity alone, might
narrow the equity--diversity trade-off observed on this dataset without
sacrificing AURORA's representation gains. Finally, the greedy merge
strategy could be replaced by more expressive
community-detection techniques, potentially guided by the EDI impact of
each candidate merge rather than by rating-profile similarity alone, to
improve both compression rate and EDI preservation simultaneously.
