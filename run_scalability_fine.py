...
k=10, theta=4.0 — mesure runtime + metriques EDI
Complementaire a run_scalability.py (1M, 500-6040)
Resultats -> scalability_100k_results.json
"""
...
SEED  = 42
SIZES = [100, 150, 200, 300, 400, 500, 700, 943]
K     = 10
THETA = 4.0
...
if __name__ == "__main__":
    ...
    # Renommer colonne gender si besoin
    if "gender" not in users_df.columns and "gender" in users_df.columns:
        pass
    ...
    results = {
        "meta": {"dataset": "MovieLens 100k (sous-ensembles)", "k": K, "theta": THETA,
    ...
    for n_users in SIZES:
        ...
        for method_name, fn in [
            ...
            t0 = time.time(); items = fn(); rt = time.time() - t0
        ...
        print(f"  {'Methode':<16} {'dE':>7} {'ILD':>7} {'temps':>10}")
        for meth in METHODS:
            v = exp[meth]
            print(f"  {meth:<16} {v['dE']:>7.4f} {v['ILD']:>7.4f} {v['elapsed_s']:>9.3f}s")

    out = os.path.join(os.path.dirname(__file__), "scalability_100k_results.json")
