...
glouton fusionne itérativement des noeuds utilisateurs tout en imposant
...
Ce mémoire s'inscrit dans le cadre du Master 2 DataScale de l'Université
Paris-Saclay (UVSQ), réalisé en séjour de recherche au Department of
Computer Science de l'University of Regina (Saskatchewan, Canada), sous la
direction du Prof. Malek Mouhoub.
...
(UVSQ). Le stage a été encadré par le \textbf{Prof. Malek Mouhoub},
enseignant-chercheur au sein de ce département.

% TODO (à préciser avec Malek Mouhoub) : nom exact du laboratoire ou du
% groupe de recherche, ses thématiques principales (par ex. intelligence
% artificielle, programmation par contraintes, systèmes multi-agents),
% ses projets en cours, et la taille/composition de l'équipe. Cette
% section est un point de départ à vérifier et compléter directement
% auprès de l'encadrant, plutôt que des détails déduits ou supposés.

\section{Missions du Département}

Le Department of Computer Science de l'University of Regina mène des
activités de recherche et d'enseignement en informatique, avec des
travaux couvrant notamment l'intelligence artificielle et l'aide à la
décision. C'est dans ce contexte que s'inscrit la problématique de
l'équité algorithmique traitée durant ce stage.

% TODO : compléter avec les axes de recherche précis du groupe d'accueil
% une fois confirmés.

\section{Cadre de la Collaboration}

Ce travail s'inscrit dans une collaboration de recherche entre
l'Université Paris-Saclay (UVSQ) et l'University of Regina, portant sur
l'équité, la diversité et l'inclusion (EDI) dans les systèmes de
recommandation. Il a donné lieu à la rédaction d'un article scientifique
soumis à une conférence internationale (format Springer LNCS), dont ce
mémoire reprend et détaille le contenu en français.
...
est devenu une préoccupation centrale dans de nombreux domaines. Les
...
l'équité en modifiant les préférences en entrée, en modifiant le
classement en sortie par re-classement, ou en régularisant l'algorithme
d'apprentissage. Le résumé de graphe, de son côté, offre un moyen
...
\[
  G = (U \cup I,\; E,\; w,\; s),
\]
où $U = \{u_1, \ldots, u_m\}$ est l'ensemble des noeuds utilisateurs, $I =
\{i_1, \ldots, i_n\}$ est l'ensemble des noeuds items (par exemple des
...
sensible $s$ est défini sur les noeuds utilisateurs dans les configurations
d'\emph{équité côté utilisateur} ($s : U \rightarrow \{F, M\}$, par
exemple MovieLens, libimseti) ou sur les noeuds items dans les
...
laquelle les noeuds sont fusionnés en \emph{super-noeuds} selon leur
similarité, et les arêtes entre groupes sont agrégées en
\emph{super-arêtes}~\cite{liu_graph_2018}. Le processus de fusion de
noeuds pour obtenir un graphe plus petit est appelé \emph{coarsening}. Le
...
analyse opère sur une métrique de graphe, regroupant les noeuds en
...
(fusion de noeuds en super-noeuds), compression de bits,
...
nous réduisons le graphe par fusion de noeuds.
...
Copeland, Ranked Pairs) aux côtés du recommandeur de base pour produire
...
candidate de noeuds utilisateurs n'est acceptée que si le graphe résumé
...
paires de noeuds utilisateurs en super-noeuds selon la similarité de leurs
profils de notation. Le résultat est un graphe résumé $G' = (U' \cup I,
E', w', s')$ avec $|U'| < |U|$, où chaque super-noeud $S \subseteq U$
représente un groupe d'utilisateurs originaux. Le poids agrégé du
super-noeud $S$ vers l'item $i$ est
\begin{equation}
  w'(S, i) =
...
niveau structurel : toute fusion de noeuds utilisateurs qui conduirait à
...
$\Delta E$ à chaque $k \in \{5,10,20\}$ sans jamais les dominer sur
l'inclusion, et c'est la seule méthode à améliorer les quatre métriques
par rapport à Borda simultanément à $k=10$. Sur l'ILD, elle dépasse Borda
...
fois plus lent que Fair Re-rank et 30 à 60 fois plus lent que Borda.
...
dans la structure du graphe : chaque fusion candidate de noeuds
...
de référence ne satisfait les trois critères EDI simultanément. Sur
MovieLens 100k, AURORA bat systématiquement les règles de vote
classiques (Borda, Weighted Borda, Condorcet) à la fois sur l'équité et
sur la diversité pour $k \in \{5,10,20\}$, avec un budget de fusion
robuste sur une plage de 30 à 60\,\% ; Fair Re-rank demeure la méthode la
plus précise sur l'équité seule, au prix d'un certain coût en diversité.
Ce constat n'est pas universel mais dépend du jeu de données : sur le
plus grand MovieLens~1M, Fair Re-rank domine AURORA sur les deux axes, ce
que nous attribuons à son vivier de candidats plus large pour le
re-classement ; sur libimseti, où l'attribut sensible se situe des deux
côtés du graphe biparti, aucune méthode --- y compris AURORA --- ne
réduit significativement l'écart d'équité, une limite que nous rattachons
à une réelle différence de comportement de notation entre les groupes
plutôt qu'à une lacune algorithmique. Sur les jeux de données côté item,
AURORA présente deux profils distincts : sur Rate My Professors, elle
échange de l'équité contre un gain important en diversité
(ILD$=0{,}808$) et en représentation féminine ($\mathrm{frac}_F=60\,\%$),
tandis que sur OpenAlex, elle améliore l'équité \emph{et} maintient la
diversité simultanément, atteignant l'écart d'équité le plus faible
observé sur les cinq jeux de données ($\Delta E = 0{,}051$) et corrigeant
le biais le plus extrême de notre évaluation.
...
critère principié --- tel qu'une analyse de front de Pareto ou une
