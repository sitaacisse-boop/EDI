...
atteint l'écart d'équité le plus faible ($\Delta E=0{,}031$) sur OpenAlex,
...
deux côtés du graphe biparti, notre méthode --- comme les règles de vote
classiques --- ne réduit pas l'écart d'équité
...
ce stage, ainsi que la Prof. Zoubida Kedad pour son suivi pédagogique. Je
...
à la revue internationale \emph{Expert Systems With Applications}
(Elsevier), dont ce mémoire reprend et détaille le contenu en
français. Le suivi
...
eux. Ce mémoire restreint la notion d'EDI à l'attribut du genre, sous sa
forme binaire (F/M) telle que disponible dans les cinq jeux de données
étudiés ; il ne traite pas d'autres attributs sensibles (âge, origine,
etc.), ni de catégories de genre non binaires. L'extension à des
attributs sensibles multiples et simultanés est laissée aux travaux
futurs (chapitre~\ref{chap:conclusion}).

La figure~\ref{fig:bipartite-example} illustre cette structure sur un
petit exemple à quatre utilisateurs et trois items, repris et détaillé
numériquement au tableau~\ref{tab:toy} (section~\ref{sec:example}).

\begin{figure}[H]
\centering
\definecolor{rosepref}{RGB}{247,180,200}
\definecolor{mauvepref}{RGB}{186,150,214}
\resizebox{0.7\textwidth}{!}{%
\begin{tikzpicture}[
  user/.style={circle, draw, minimum size=9mm, font=\small},
  userF/.style={user, fill=rosepref},
  userM/.style={user, fill=mauvepref},
  item/.style={circle, draw, minimum size=9mm, font=\small, fill=orange!15},
  edgelabel/.style={font=\scriptsize, fill=white, inner sep=1pt},
  legtext/.style={font=\scriptsize, align=left, text width=3.4cm},
  callout/.style={font=\scriptsize\itshape, align=left, text width=3.0cm},
  arrowline/.style={-{Latex[length=2mm]}, gray, thick, dashed}
]
  % en-têtes de colonnes
  \node[font=\small\bfseries] (hdrU) at (0, 5.4) {Utilisateurs $U$};
  \node[font=\small\bfseries] (hdrI) at (5, 5.4) {Items $I$};

  % nœuds utilisateurs (colonne de gauche) : groupe F (rose) / groupe M (mauve)
  \node[userF] (u1) at (0, 4.5) {$u_1$};
  \node[userM] (u2) at (0, 3.0) {$u_2$};
  \node[userM] (u3) at (0, 1.5) {$u_3$};
  \node[userM] (u4) at (0, 0.0) {$u_4$};

  % nœuds items (colonne de droite)
  \node[item] (i1) at (5, 4.5) {$i_1$};
  \node[item] (i2) at (5, 2.25) {$i_2$};
  \node[item] (i3) at (5, 0.0) {$i_3$};

  % arêtes pondérées : w(u,i) = note attribuée
  \draw (u1) -- node[edgelabel] (w11) {5} (i1);
  \draw (u1) -- node[edgelabel] {2} (i2);
  \draw (u2) -- node[edgelabel] (w21) {1} (i1);
  \draw (u2) -- node[edgelabel] {5} (i2);
  \draw (u2) -- node[edgelabel] {3} (i3);
  \draw (u3) -- node[edgelabel] {4} (i2);
  \draw (u3) -- node[edgelabel] {5} (i3);
  \draw (u4) -- node[edgelabel] {2} (i1);
  \draw (u4) -- node[edgelabel] {5} (i2);
  \draw (u4) -- node[edgelabel] {4} (i3);

  % --- légende des couleurs, placée à droite du graphe ---
  \node[userF, minimum size=5mm, font=\tiny] (legF) at (7.0, 4.5) {};
  \node[legtext, right=2mm of legF] {Groupe utilisateur $F$ : Femme};

  \node[userM, minimum size=5mm, font=\tiny] (legM) at (7.0, 3.6) {};
  \node[legtext, right=2mm of legM] {Groupe utilisateur $M$ : Homme};

  \node[item, minimum size=5mm, font=\tiny] (legI) at (7.0, 2.7) {};
  \node[legtext, right=2mm of legI] {Item $i \in I$};

  % --- flèches annotant l'arête, le poids, et l'attribut sensible ---
  \node[callout] (ann-e) at (-3.4, 3.6) {une arête\\$(u,i) \in E$ :\\$u$ a noté $i$};
  \draw[arrowline] (ann-e.east) to[bend right=15] (w21.west);

  \node[callout] (ann-w) at (-3.4, 1.2) {le poids\\$w(u,i)$ :\\la note attribuée};
  \draw[arrowline] (ann-w.east) to[bend right=15] (w11.south west);

  \node[callout] (ann-s) at (7.0, 1.4) {attribut sensible\\$s(u) \in \{F, M\}$ :\\couleur du nœud};
  \draw[arrowline] (ann-s.north) to[bend left=15] (legF.south);
\end{tikzpicture}%
}
\caption{Exemple de graphe de préférences biparti attribué et pondéré
$G = (U \cup I, E, w, s)$ : quatre utilisateurs (dont une femme, en rose,
et trois hommes, en mauve) notent jusqu'à trois items. Les annotations
indiquent les éléments de la notation formelle : un sommet utilisateur
$u \in U$, un sommet item $i \in I$, une arête $(u,i) \in E$, son poids
$w(u,i)$ (la note attribuée), et l'attribut sensible $s$ porté par les
utilisateurs. L'absence d'arête entre $u_1$ et $i_3$, ou entre $u_3$ et
$i_1$, signifie que la note n'a pas été observée. Cet exemple est repris
au tableau~\ref{tab:toy}.}
...
post-hoc laisse la voie structurelle ouverte. Nous ne l'implémentons pas
comme référentiel expérimental : SCRUF-D exige une fonction d'utilité par
préoccupation d'équité et un mécanisme de négociation inter-agents (vote
de Borda, Copeland ou Ranked Pairs entre agents), à concevoir et calibrer
séparément pour chacun de nos quatre corpus --- une contribution
méthodologique à part entière, distincte de notre protocole à métriques
EDI fixes. Fair Re-rank, notre référentiel de post-traitement, reste
comparable dans l'esprit (un ajustement du classement final sous
contrainte d'équité) sans requérir cette architecture multi-agents ; une
implémentation fidèle de SCRUF-D est laissée comme référentiel
prioritaire pour un travail futur.
...
par défaut sur l'échelle 1--5 et comparons avec $\theta = 3{,}5$, seuil
adopté par Aird et al.~\cite{aird_social_2024} pour définir un item
« aimé » sur MovieLens (note $>3$) ; ce seuil est recalibré par jeu de
données pour préserver la même sémantique sur des échelles différentes
(voir chapitre~\ref{chap:validation}, section Jeux de Données).
...
genre de l'utilisateur qui note (équité côté utilisateur). Pour les
expériences principales de coarsening sous contraintes EDI, nous
utilisons un sous-échantillon aléatoire de 1\,000 évaluateurs (97\,707
notes, tableau~\ref{tab:datasets-overview}) ; la
section~\ref{sec:libimseti-limitation} analyse en complément le graphe
complet à 17,3 M d'arêtes pour caractériser le comportement de notation
par paire de genres. Nous fixons $k \in \{5, 10, 20\}$.
...
\begin{table}[H]
\centering
\caption{Vue d'ensemble des cinq jeux de données.}
\label{tab:datasets-overview}
\begin{tabular}{lccccccc}
\hline
Jeu de données & Domaine & $|U|$ & $|I|$ & $|E|$ & Échelle & Équité & $\alpha_F$ \\
\hline
MovieLens 100k    & Films        & 943    & 1\,682  & 100\,000    & 1--5  & user-side & 29\,\% \\
MovieLens 1M      & Films        & 6\,040 & 3\,706  & 1\,000\,209 & 1--5  & user-side & 28\,\% \\
libimseti.cz      & Rencontres   & 1\,000 & 32\,677 & 97\,707     & 1--10 & user-side & 50\,\% \\
Rate My Prof.     & Éducation    & 1\,981 & 59\,066 & 150\,800    & 1--5  & item-side & 51\,\%* \\
OpenAlex          & Académique   & 99     & 904     & 2\,124      & 1--5  & item-side & 19\,\% \\
\hline
\end{tabular}
\vspace{2pt}
\begin{footnotesize}
* $\alpha_F$ = proportion de femmes dans les items (professeurs
recommandés), pas dans les utilisateurs.
\end{footnotesize}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig_datasets_overview_full_fr}
\caption{Vue d'ensemble des cinq jeux de données : taille des corpus
(échelle log) et déséquilibre de genre $\alpha_F$ par côté d'équité
(haut), et répartition des notes pour chaque jeu de données (bas). Pour
Rate My Professors, la note est une moyenne (cours, professeur) continue
sur 1--5, regroupée en huit intervalles de largeur 0,5.}
...
et $\theta = 7{,}0$ pour libimseti ---
équivalent sémantique du seuil MovieLens sur l'échelle 1--10 de la
plateforme, plutôt qu'une mise à l'échelle linéaire stricte. Pour les
jeux de données à équité côté item (Rate My Professors, OpenAlex), nous
rapportons $\Delta E$, l'ILD, et $\mathrm{frac}_F$ --- la fraction
d'items féminins dans la liste top-$k$, avec $\theta = 4{,}0$ pour Rate
My Professors (même échelle 1--5 que MovieLens) et $\theta = 1{,}0$ pour
OpenAlex, seuil minimal justifié par la forte parcimonie de la matrice
de notes (note $= \min(\text{nb papiers}, 5)$ ; la plupart des auteurs
n'ont qu'un seul papier par venue, voir figure~\ref{fig:datasets-overview}).
Les tolérances de
...
tous les jeux de données : ces trois valeurs sont usuelles dans
l'évaluation des systèmes de recommandation top-$k$ et couvrent une
liste courte ($k=5$), la taille la plus fréquemment rapportée dans la
littérature sur l'équité en recommandation, notamment par Aird et
al.~\cite{aird_exploring_2023} ($k=10$), et une liste plus longue ($k=20$)
permettant de vérifier que les tendances observées ne sont pas un
artefact d'une liste courte. Nous rapportons le taux de compression
$|U'|/|U|$ aux côtés de toutes les métriques EDI.
...
score d'inclusion pour au moins un groupe à chaque $k$ ; elle améliore
les quatre métriques par rapport à Borda simultanément à $k=10$
(Condorcet y parvient également). À $k=20$, ce succès s'étend aux
\emph{trois} règles de vote à la fois (Borda, Condorcet, et Weighted
Borda simultanément) --- un résultat plus complet qu'à $k=10$, ce qui
explique pourquoi $k=20$ est retenu comme configuration globale pour
MovieLens 100k dans la synthèse inter-domaines (tableau~\ref{tab:synthesis}).
Sur l'ILD, elle dépasse Borda
...
tandis qu'AURORA prend 3,8 à 4,0\,s sur MovieLens 100k --- environ 4,0 à
4,4 fois plus lent que Fair Re-rank et 5 à 6 fois plus lent que Borda.
...
$\Delta E$ bas (0,139) parmi les méthodes non dégénérées ; AURORA obtient
la meilleure ILD parmi les méthodes à inclusion non triviale (0,761,
devant Borda, Weighted Borda et Condorcet, entre 0,716 et 0,746) mais,
comme les méthodes de vote, ne réduit pas significativement $\Delta E$
par rapport à Borda (1,232 contre 1,301, tous deux bien au-delà de 1,2).
Cette limite --- propre à ce seul jeu de
...
fusion de 30\,\% à 60\,\% de $|U|$. Ce constat tient à $k=5$ et
$k=20$ également, avec une nuance notable à $k=5$ : Borda, Weighted
Borda, Condorcet et AURORA convergent tous exactement vers
$\Delta E = 1{,}138$, tandis que Fair Re-rank domine seul sur les deux
axes ($\Delta E = 0{,}0004$, ILD$=0{,}762$) --- un cas unique parmi les
trois $k$ testés. À $k=20$, Fair Re-rank perd une grande partie de son
avantage ($\Delta E = 0{,}315$) tandis qu'AURORA prend la tête en
diversité parmi les approches non dégénérées (ILD$=0{,}781$).
...
$\Delta E$ (0,112) plus élevé que Borda (0,065) : sur ce corpus, la
...
diversité bien plus faible (ILD$=0{,}276$), montrant que le re-classement
a posteriori ne peut pas optimiser diversité et représentation aussi
bien que le \emph{coarsening} structurel. Le compromis équité/diversité
d'AURORA se déplace avec $k$ : à $k=5$, quatre méthodes sur six
atteignent $\Delta E = 0$ exactement, tandis qu'AURORA a le seul écart
non nul (0,042) mais aussi de loin la meilleure représentation
féminine ($\mathrm{frac}_F=0{,}80$) ; à $k=20$, c'est au contraire Borda
qui affiche le pire $\Delta E$ du tableau (0,139), qu'AURORA (0,078)
dépasse, tout en gardant la meilleure ILD (0,818).
...
(0,031, 56\,\% de mieux qu'Average Score) tout en maintenant une
représentation féminine comparable ($\mathrm{frac}_F=0{,}194$) ---
...
toutefois dépendants du domaine. Le biais reste sévère aux trois $k$
testés : à $k=5$, même Average Score échoue ($\Delta E=1{,}23$, comme
Borda) faute d'assez de candidats pour le diluer, et seuls Fair Re-rank
et AURORA restent équitables. À $k=20$, le biais de Borda s'atténue
légèrement ($\Delta E=0{,}907$, 5\,\% d'autrices contre 0\,\% à $k=5$/10) ;
AURORA conserve dans tous les cas le meilleur $\Delta E$ du tableau
(0,058 à $k=5$ et $k=20$, 0,031 à $k=10$).
...
recommandés sont eux-mêmes des personnes. Pour comprendre pourquoi
AURORA, comme les méthodes de vote, ne réduit pas $\Delta E$ ici, nous avons
...
Gras : meilleure(s) valeur(s) par colonne (toutes les valeurs ex æquo
sont mises en gras).}
\label{tab:rmp}
\centering
\begin{tabular}{lccc}
...
($k=10$, équité côté item ; $\alpha_F \approx 19\,\%$). Gras :
meilleure(s) valeur(s) par colonne (toutes les valeurs ex æquo sont
mises en gras).}
\label{tab:oalex}
\centering
\begin{tabular}{lccc}
...
$|U|$) sur MovieLens 1M ($k=20$) et Rate My Professors ($k=10$), les deux
jeux de données où l'effet de la compression est le plus visible.
...
s'améliorent quand le budget augmente --- nettement pour $\Delta E$
(0,332 à 30\,\% $\to$ 0,185 à 60\,\%, soit une réduction de 44\,\%) et
marginalement pour l'ILD (0,456 $\to$ 0,470) --- la méthode est robuste
au choix exact du budget. Sur Rate My Professors, l'ILD reste
...
($\approx 0{,}065$) : le compromis équité/diversité décrit en
section~\ref{sec:libimseti-limitation} tient quel que soit le budget
choisi entre 30 et 60\,\%.

Les résultats de MovieLens 100k à $k=10$ (tableau~\ref{tab:k10}) sont-ils
stables sous variation de l'échantillon d'utilisateurs~? Un
sous-échantillonnage sans remise (90\,\% de $|U|$, 30 répétitions)
confirme que oui : AURORA bat Borda et Condorcet sur $\Delta E$ dans
28/30 tirages et sur l'ILD dans 30/30 tirages (Weighted Borda : 24/30 et
30/30), avec un écart-type faible sur les deux métriques ($\Delta E =
0{,}417 \pm 0{,}035$, ILD $= 0{,}391 \pm 0{,}004$) --- l'avantage
d'AURORA sur les règles de vote classiques n'est pas un artefact du
tirage unique rapporté au tableau~\ref{tab:k10}. Fair Re-rank, en
revanche, reste devant AURORA sur les deux métriques dans les 30 tirages
sans exception, confirmant que son avantage sur ce corpus est lui aussi
robuste.
...
RMP      & Avg Score / W.Borda / F.R. & Avg Score & \textbf{AURORA} & Compromis : AURORA (div.) / F.R. (éq.) \\
...
ici est à $k=20$, où elle reste compétitive sur l'inclusion féminine
(0,294, quasi identique au meilleur, Weighted Borda à 0,295) : à cette
échelle, son avantage se déplace vers l'équilibrage entre groupes plutôt
que la seule minimisation de
...
calculée. Pour valider cette approche, nous l'avons évaluée sur cinq
jeux de données couvrant quatre domaines : aucune méthode de référence
n'y satisfait les trois critères EDI simultanément, tandis qu'AURORA bat
systématiquement les règles de vote classiques
sur l'équité et la diversité, et égale ou dépasse Fair Re-rank sur les
corpus plus petits ou à équité côté item --- notamment sur OpenAlex, où
elle atteint l'écart d'équité le plus faible observé ($\Delta E =
0{,}031$) tout en corrigeant le biais le plus extrême de notre
évaluation. Deux limites structurelles se dégagent cependant : sur le
plus grand MovieLens~1M, Fair Re-rank domine AURORA sur les deux axes,
probablement grâce à un vivier de candidats plus large pour le
re-classement ; et sur libimseti, où l'attribut sensible existe des deux
côtés du graphe, AURORA --- comme les règles de vote classiques --- ne
réduit pas l'écart d'équité, ce que nous rattachons à une différence
réelle de comportement entre les groupes plutôt qu'à une lacune
algorithmique. Pris ensemble, ces résultats soutiennent une affirmation
plus précise qu'« AURORA est uniformément meilleure » : intégrer des
contraintes EDI dans la structure de \emph{coarsening} surpasse
systématiquement les règles de vote non contraintes, et égale ou
améliore le re-classement a posteriori sur les jeux de données plus
petits ou à équité côté item, mais une base d'utilisateurs suffisamment
grande ou un domaine à préférences réciproques et dépendantes de
l'attribut peut favoriser une approche a posteriori ou exposer une
limite réelle de tout cadre d'agrégation à classement unique. Cette
nuance constitue en soi une contribution : elle clarifie les conditions
dans lesquelles un résumé structurel sous contraintes est préférable au
re-classement, plutôt que d'affirmer une supériorité inconditionnelle.
